...
function FeedCard({
  ...
  return (
    <div className="w-[60%] mx-auto p-6 bg-[#F5F0E1] border border-[#DCCFC0] rounded-xl shadow-lg hover:shadow-2xl transition duration-300 flex flex-col items-center text-center relative">
      ...
      <p className="text-md text-[#6B4226] font-semibold absolute top-4 left-4">
        Author: {author}
      </p>

      {/* Published Date */}
      <p className="text-md text-[#8E7C3D] italic absolute top-4 right-4">
      ...
      {/* Content */}
      <div className="bg-[#FFF9F1] px-4 py-6 rounded-lg shadow-md border border-[#EADBCB] text-left w-1/2">
        {content.split("\n").map((paragraph, index) => (
          <p
            key={index}
